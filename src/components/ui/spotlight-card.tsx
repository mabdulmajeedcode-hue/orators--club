...
const GlowCard: React.FC<GlowCardProps> = ({ children, className = '', glowColor = 'green' }) => {
  ...
  const beforeAfterStyles = `
    ...
    [data-glow] [data-glow] { position: absolute; inset: 0; will-change: filter; opacity: var(--outer, 1); border-radius: calc(var(--radius) * 1px); filter: blur(calc(var(--border-size) * 10)); background: none; pointer-events: none; border: none; }
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          ['--radius' as any]: '14',
          ['--border' as any]: '2',
          ['--backdrop' as any]: 'hsl(0 0% 60% / 0.08)',
        ...
        className={`rounded-2xl relative shadow-[0_1rem_2rem_-1rem_black] backdrop-blur-[5px] transition-transform duration-300 hover:-translate-y-2 hover:scale-[1.02] ${className}`}
      >
        <div data-glow></div>
        {children}
      </div>
