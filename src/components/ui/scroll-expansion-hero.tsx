...
const ScrollExpandMedia = ({ mediaType = 'video', mediaSrc, posterSrc, bgImageSrc, title, date, scrollToExpand, textBlend, children }: ScrollExpandMediaProps) => {
  ...
  useEffect(() => {
    const handleWheel = (e: any) => {
      if (mediaFullyExpanded && e.deltaY < 0 && window.scrollY <= 5) { setMediaFullyExpanded(false); e.preventDefault(); }
        ...
        <div className="relative w-full flex flex-col items-center min-h-[100dvh]">
          ...
          <div className="container mx-auto flex flex-col items-center justify-start relative z-10">
            <div className="flex flex-col items-center justify-center w-full h-[100dvh] relative">
              <div className="absolute z-0 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-2xl" style={{ width: `${mediaWidth}px`, height: `${mediaHeight}px`, maxWidth: '95vw', maxHeight: '85vh', boxShadow: '0px 0px 50px rgba(0,0,0,0.3)' }}>
                {mediaType === 'video' ? (
                  ...
                  <div className="relative w-full h-full">
                    <img src={mediaSrc} alt={title || 'Hero'} className="w-full h-full object-cover rounded-xl" />
                ...
                <div className="flex flex-col items-center text-center relative z-10 mt-4">
                  ...
                  {scrollToExpand && <p className="text-white/60 font-medium" style={{ transform: `translateX(${textTranslateX}vw)` }}>{scrollToExpand}</p>}
                </div>
              </div>
              <div className={`flex items-center justify-center text-center gap-4 w-full relative z-10 flex-col ${textBlend ? 'mix-blend-difference' : ''}`}>
                <motion.h2 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white" style={{ transform: `translateX(-${textTranslateX}vw)` }}>{firstWord}</motion.h2>
                <motion.h2 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white" style={{ transform: `translateX(${textTranslateX}vw)` }}>{restOfTitle}</motion.h2>
              </div>
            </div>
