...
import { Play, Loader2, ExternalLink, ChevronDown, ChevronUp } from "lucide-react";
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              {podcasts.map((ep: any, i: number) => (
                <PodcastCard key={ep.id} ep={ep} i={i} />
              ))}
            </div>
          )}
        </div>
...
const PodcastCard = ({ ep, i }: { ep: any; i: number }) => {
    ...
    >
      <div className="flex flex-col sm:flex-row">
        <div className="flex-1 p-6 flex flex-col justify-center order-2 sm:order-1">
          ...
          {ep.description && (
            <p className={`text-sm text-muted-foreground mb-4 leading-relaxed ${expanded ? "" : "line-clamp-3"}`}>
              {ep.description}
            </p>
          )}
          <AnimatePresence>
            {expanded && ep.description && ep.description.length > 150 && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
